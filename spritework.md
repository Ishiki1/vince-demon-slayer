...
- **Blacksmith** uses `blacksmith-bg.png` (1536x1058 source, stretched to 800x600) with invisible hotspots from `blacksmithscene-hotspots-800x600.json` (19 hotspots). Combined three-column view: Repair (left, `repair-1`..`repair-5`), Craft (center, `craft-1`..`craft-5`), Upgrade (right, `upgrade-1`..`upgrade-5`). Panel reference zones (`repair-panel`, `craft-panel`, `upgrade-panel`) and a `back` button at bottom-right. Icon-only item slots with hover tooltips and click-to-act. `UpgradeScene` has been removed; upgrade logic lives in BlacksmithScene.
- **Mine** uses `mine-bg.png` (1536x1024 source, stretched to 800x600) with a single invisible hotspot from `minescene-hotspots-800x600.json`. Hotspot: `minecart` (100,95 600x440) covers the cart, tracks, and cave entrance. Tooltip on hover shows the mining action label.
...
| Spider idle | `assets/goons/spider_idle_512x512_sheet.png` | `spider_idle_sheet` |
| Spider attack | `assets/goons/spider_attack_512x512_sheet.png` | `spider_attack_sheet` |
...
| Toad idle | `assets/goons/toad_idle_512x512_sheet.png` | `toad_idle_sheet` |
| Toad attack | `assets/goons/toad_attack_512x512_sheet.png` | `toad_attack_sheet` |
| Witch idle | `assets/goons/witch_idle_512x512_sheet.png` | `witch_idle_sheet` |
| Witch attack | `assets/goons/witch_attack_512x512_sheet.png` | `witch_attack_sheet` |
