...
- **Event** uses `EventScene-bg.png` (1536x1024 source, stretched to 800x600). When background art is active, `EventScene` now shifts the random-event copy block down by `100px` from the original art anchors (`titleY 194`, `bodyY 242`, `choiceStartY 354`, `resultY 386`, `merchantGoldY 278`, `merchantRowStartY 332`) so the text clears the top frame.
