# Dry-run: iter_007 — case_01 — Frost Spider (goon-generator skill only)

**Iteration 7 change:** After the **Attack prompt template** (bracket placeholder version), the skill adds **Attack-type motion arc examples** — three concrete 8-frame beats agents map to creature attacks: **melee lunge** (ready → coil → explosive lunge LEFT → impact → recoil → recovery), **vertical slam** (arms overhead → DOWN and toward LEFT → debris/shockwave → slow rise), **ranged spell/projectile** (gather → peak energy → release TOWARD LEFT → recoil → steady). **Iteration 6** — Frame Count Guidance between Phase 3 and Phase 4, Creature-Specific Adaptations, eight-frame templates, facing gates — is otherwise unchanged.
...
<!-- SCORES
{
  ...
  "reasoning": {
    "Attack Animation Quality": "+6 vs iter_006: melee-lunge example gives an exact 8-beat pounce arc (coil → explosive lunge LEFT → impact → recoil → settle) so the spider prompt is no longer inferred only from generic brackets.",
    "Facing Direction Enforcement": "Unchanged vs iter_006; same gates and anti-patterns.",
    "Green Cleanup Verification": "Unchanged vs iter_006; thin-limb and outline rows unchanged.",
    "Frame Count Adequacy": "Unchanged vs iter_006 at 70.",
    "Phase Completeness": "Unchanged vs iter_006 at 80; examples are an inline subsection after the template, not a new pipeline phase.",
    "Prompt Adaptability": "+2 vs iter_006: mapping creature attack class to one of three arcs demonstrates type-aware prompting without new phases."
  }
}
-->

**Weighted case score (AAQ 25%, FDE 20%, GCV 20%, FCA 15%, PC 10%, PA 10%):**  
78×0.25 + 70×0.20 + 68×0.20 + 70×0.15 + 80×0.10 + 67×0.10 = 19.5 + 14 + 13.6 + 10.5 + 8 + 6.7 = **72.30**
