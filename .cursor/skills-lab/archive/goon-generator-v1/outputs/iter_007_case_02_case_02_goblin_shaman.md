# Dry-run: iter_007 — case_02 — Goblin Shaman (goon-generator skill only)

**Iteration 7 change:** After the **Attack prompt template**, the skill adds **Attack-type motion arc examples**: **melee lunge**, **vertical slam**, and **ranged spell/projectile**, each with an 8-frame F1–F8 beat list that replaces vague bracket placeholders. **Iteration 6** — Frame Count Guidance, Creature-Specific Adaptations, eight-frame templates, facing gates — is otherwise unchanged.
...
<!-- SCORES
{
  ...
  "reasoning": {
    "Attack Animation Quality": "+6 vs iter_006: ranged example spells out gather → peak → launch LEFT → flight → recoil → steady, matching staff blast beats without guessing from generic brackets.",
    ...
    "Phase Completeness": "Unchanged vs iter_006 at 80.",
    "Prompt Adaptability": "+2 vs iter_006: three archetypes make it obvious when to deviate from horizontal lunge language (here: caster recoil and bolt path)."
  }
}
-->

**Weighted case score (AAQ 25%, FDE 20%, GCV 20%, FCA 15%, PC 10%, PA 10%):**  
78×0.25 + 70×0.20 + 73×0.20 + 70×0.15 + 80×0.10 + 67×0.10 = 19.5 + 14 + 14.6 + 10.5 + 8 + 6.7 = **73.30**
