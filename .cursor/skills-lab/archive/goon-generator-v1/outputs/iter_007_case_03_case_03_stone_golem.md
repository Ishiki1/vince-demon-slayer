# Dry-run: iter_007 — case_03 — Stone Golem (goon-generator skill only)

**Iteration 7 change:** After the **Attack prompt template**, the skill adds **Attack-type motion arc examples**, including a **vertical slam** arc: arms overhead → slam **DOWN and toward LEFT** → debris/shockwave → **slow** recovery — addressing golem awkwardness when only a generic lunge template was available. **Iteration 6** — Frame Count Guidance (including heavy-creature `frameRate` note), Creature-Specific Adaptations, eight-frame templates, facing gates — is otherwise unchanged.
...
<!-- SCORES
{
  ...
  "reasoning": {
    "Attack Animation Quality": "+7 vs iter_006: vertical-slam example gives an explicit overhead → down-left → shockwave → slow-rise arc; removes ambiguity between horizontal lunge language and stomp reads.",
    "Facing Direction Enforcement": "Unchanged vs iter_006.",
    "Green Cleanup Verification": "Unchanged vs iter_006.",
    "Frame Count Adequacy": "Unchanged vs iter_006 at 68.",
    "Phase Completeness": "Unchanged vs iter_006 at 75.",
    "Prompt Adaptability": "+3 vs iter_006: slam-class creatures now have a first-class arc alongside lunge and ranged, improving type-to-prompt mapping for heavy bruisers."
  }
}
-->

**Weighted case score (AAQ 25%, FDE 20%, GCV 20%, FCA 15%, PC 10%, PA 10%):**  
78×0.25 + 70×0.20 + 63×0.20 + 68×0.15 + 75×0.10 + 68×0.10 = 19.5 + 14 + 12.6 + 10.2 + 7.5 + 6.8 = **70.60**
